...
<body>
	<div id="pdc">
		<table id="product">
			<tr>
				<td><a href="#" id="order" value="ASC"
					onClick="list('${map.sPrd}', '1', 'ASC')">낮은가격</a> <a href="#"
					id="order" value="DESC" onClick="list('${map.sPrd}', '1', 'DESC')">높은가격</a>
				</td>
				<td><input type="radio" name="brand" value="nike"> <input
					type="radio" name="brand" value="addias">
			</tr>
			<c:if test="${(fn:length(map.list)) eq 0}">
				<tr>
					<td colspan="4">상품내역이 없습니다.</td>
				</tr>
			</c:if>
			...
			<c:if test="${(fn:length(map.list)) ne 1}">
				<tr>
					<td colspan="4" align="center"><c:if
							...
							<a href="#" onClick="list('${map.sPrd}', '1', '${map.order}')">[처음]</a>
						</c:if> <c:if test="${map.pager.curBlock > 1}">
							<a href="#"
								onClick="list('${map.sPrd}', '${map.pager.prevPage}', '${map.order}')">[이전]</a>
							...
							<c:choose>
								...
								<c:otherwise>
									<a href="#"
										onClick="list('${map.sPrd}', '${num}', '${map.order}')">${num}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach> <c:if test="${map.pager.curBlock < map.pager.totBlock}">
							<a href="#"
								onClick="list('${map.sPrd}', '${map.pager.nextPage}', '${map.order}')">[다음]</a>
						</c:if> <c:if test="${map.pager.curBlock < map.pager.totPage}">
							<a href="#"
								onClick="list('${map.sPrd}', '${map.pager.totPage}', '${map.order}')">[끝]</a>
						</c:if></td>
				</tr>
			</c:if>
		</table>
